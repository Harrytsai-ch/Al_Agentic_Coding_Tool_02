 ...
 * /api/orders/{id}/ecpay:
 *   post:
 *     summary: 取得綠界 AIO 送單參數
 *     description: 為訂單產生一組新的綠界 MerchantTradeNo（每次皆重新產生，避免重複送單被綠界拒絕），並回傳自動送單所需的 actionUrl 與參數。
 *     tags: [Orders]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
router.post('/:id/ecpay', (req, res) => {
  ...
  const items = db
    .prepare('SELECT product_name FROM order_items WHERE order_id = ?')
    .all(order.id);
  ...
  const { actionUrl, params } = ecpay.buildCheckoutParams({
    ...
    itemNames,
    tradeDesc: `花店訂單 ${order.order_no}`,
  });

  res.json({
    data: { actionUrl, params, merchantTradeNo },
    error: null,
    message: '成功',
  });
});
 ...
 *     summary: 主動向綠界查詢付款結果
 *     description: |
 *       本地環境無法接收綠界 Server Notify（ReturnURL），故改為主動呼叫
 *       QueryTradeInfo/V5 API 驗證付款結果，並同步更新訂單狀態。
 *     tags: [Orders]
 *     security:
 *       - bearerAuth: []
 *     parameters:
...
router.get('/:id/payment-status', async (req, res, next) => {
  try {
    ...
    // Already settled — return current state without re-querying.
    if (order.status !== 'pending') {
      const items = db.prepare('SELECT * FROM order_items WHERE order_id = ?').all(order.id);
      return res.json({
        data: { order: { ...order, items }, tradeStatus: order.status === 'paid' ? '1' : '10200095' },
        error: null,
        message: '成功',
      });
    }
    ...
    let trade;
    try {
      trade = await ecpay.queryTradeInfo(order.merchant_trade_no);
    } catch (err) {
      return res.status(502).json({
        data: null,
        error: 'ECPAY_QUERY_FAILED',
        message: err.message || '綠界查詢失敗',
      });
    }
    ...
    if (tradeStatus === '1') {
      db.prepare(
        'UPDATE orders SET status = ?, ecpay_trade_no = ?, payment_type = ?, paid_at = ? WHERE id = ? AND status = ?'
      ).run('paid', trade.TradeNo || null, trade.PaymentType || null, trade.PaymentDate || null, order.id, 'pending');
    } else if (tradeStatus === '10200095') {
      db.prepare('UPDATE orders SET status = ? WHERE id = ? AND status = ?').run('failed', order.id, 'pending');
    }
    ...
    res.json({
      data: { order: { ...updated, items }, tradeStatus, tradeNo: trade.TradeNo || null, paymentType: trade.PaymentType || null },
      error: null,
      message: '成功',
    });
  } catch (err) {
